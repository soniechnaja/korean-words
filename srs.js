...
//    (карточка → выбор перевода), при достижении "3-го уровня" слово считается
//    выученным и уходит в очередь повторения.
...
const NEW_WORD_STAGE_MAX = 2; // 1 = карточка, 2 = выбор перевода; дальше — выпуск в повторение
    ...
    if (isCorrect) {
      ...
      if (srs.stageStreak >= STAGE_STREAK_TO_ADVANCE) {
        ...
        if (srs.stage > NEW_WORD_STAGE_MAX) {
          // Дошло до 3-го уровня — "выучено сегодня", уходит в очередь повторения.
          srs.learned = true;
          srs.reviewStep = 0;
